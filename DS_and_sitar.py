...
# this  fixes missing distutils.version for TensorFlow Probability
...
epp = [200,200, 200] # [5000,8000, 5000] Épocas para cada fase de entrenamiento
...
rand_eff_training = pd.DataFrame(rand_eff_training, columns=["a_DS", "b_DS", "c_DS"])

rand_eff_val = model_deep(Y_val).numpy()
rand_eff_val = pd.DataFrame(rand_eff_val, columns=["a_DS", "b_DS", "c_DS"])


print(df1_val.head())
print(df1_train.head())
print(cov_real_2.head())

print(fixed_est_df.head())
print(rand_eff_training.head())
print(rand_eff_val.head())
...
with localconverter(default_converter):
    globalenv['num_seg'] = num_seg
    globalenv['data_s'] = data_s

# --- clean DataFrame ---
df_training = df.loc[df["id"] <= t_s].copy()
df_val = df.loc[df["id"] > t_s].copy()

# --- send to R ---
with localconverter(pandas2ri.converter):
    ro.globalenv["df_training"] = ro.conversion.py2rpy(df_training)


# --- inspect in R ---
ro.r('str(df_training)')
...
# Load data
print(head(df_training))

# Fit SITAR model correctly
fit_sitar <- try(sitar(y=y, x= age , id= id, data = df_training, df = num_seg), silent=TRUE)

fixed_sit=data.frame(fixed_eff_sitar=fit_sitar$coefficients$fixed[c("a", "b","c")],coef=c("a", "b","c"),df=num_seg,nind= data_s)
print(head(fixed_sit))

random_sit=fit_sitar$coefficients$random

y_estim_sitar=data.frame(df=num_seg,nind= data_s,id=df_training$id, x=df_training$age,y_estim_sitar=fitted(fit_sitar))
print(head(y_estim_sitar))

cov_sitar=  data.frame(cov_est_sitar=as.numeric(VarCorr(fit_sitar)[,1]), nind=data_s,  df=num_seg, coef=c("sigma[a]^2","sigma[b]^2","sigma[c]^2", "sigma"))
print(head(cov_sitar))
print(paste0("Fin sitar R"))
''')


def r_df_to_pandas(name: str):
    r_obj = ro.globalenv[name]
    with localconverter(pandas2ri.converter):
        return ro.conversion.rpy2py(r_obj)

fixed_sit_py     = r_df_to_pandas('fixed_sit')
y_estim_sitar_py = r_df_to_pandas('y_estim_sitar')
cov_sitar_py     = r_df_to_pandas('cov_sitar')
random_sit_py  = r_df_to_pandas('random_sit')


rand_eff_training = pd.concat([rand_eff_training, random_sit_py], axis=1)
print(rand_eff_training)

y_estim_sitar_py['y_estim_DS'] = y_val_pred_np + Y_mean
fixed_sit_py['fixed_est']= fixed_est_list
