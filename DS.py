...
def plot_comparison(X_exact, Y_exact, Y_estimation, title=None, save_path=None):
    ...
    plt.scatter(X_np, Y_np, marker='o', alpha=0.2, label='Exact')
    plt.scatter(X_np, Yhat_np, marker='>', alpha=1.0, label='Estimation')
    if title:
        plt.title(title)
    plt.ylabel("Y")
...
def plot_loss(his_loss_train, his_loss_val, chain, lr_list, n_id=500, obs=20, num_seg=5):
    ...
    if chain <= 0 or chain > len(train_arr):
        chain = len(train_arr)
    plt.semilogy(train_arr[-chain:], label='Training Loss')
    plt.semilogy(val_arr[-chain:], label='Validation Loss')

    plt.title(f'n_id={n_id} Obs={obs} LR={lr_list}')
    plt.ylabel("log-Loss")
    plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.1),
               fancybox=True, shadow=True, ncol=2)
...
#  (se hace con los datos CENTRADOS tal como estás trabajando)
# ================================
# Construimos la matriz de bases B evaluada en TODOS los puntos de X_training
x_vec = X_training.numpy().ravel().astype(np.float32)          # [N]
x_vec_tensor = tf.constant(x_vec, dtype=tf.float32)

basis_count = bspline_layer.num_bases
B_cols = []
for i in range(basis_count):
    bi = bspline_layer.bspline_basis_tensor(x_vec_tensor, i, degree).numpy()  # [N]
    B_cols.append(bi)
B = np.stack(B_cols, axis=1).astype(np.float32)                # [N, nb]

# Vector objetivo (Y centrada) apilado
y_vec = Y_training.numpy().ravel().astype(np.float32)[:, None] # [N, 1]
nb = B.shape[1]

# Matriz de segunda diferencia para suavidad
...
# Sistema normal regularizado: (BᵀB + λ DᵀD + εI) cp = Bᵀ y
A = B.T @ B + lam * (D.T @ D) + 1e-6 * np.eye(nb, dtype=np.float32)
c = B.T @ y_vec
try:
    cp_init = np.linalg.solve(A, c).astype(np.float32)  # [nb, 1]
except np.linalg.LinAlgError:
    # Fallback robusto por si A es mal condicionada
    cp_init, *_ = np.linalg.lstsq(A, c, rcond=None)
    cp_init = cp_init.astype(np.float32)

# Asignamos los puntos de control INICIALES al MISMO layer (se usarán en DeepSITAR)
bspline_layer.control_points.assign(cp_init)

# (Opcional) ploteo de la curva B-spline pre-entrenada
...
for i in range(basis_count):
    ...
    plt.plot(x_values_pre, basis_y, linestyle='--', alpha=0.5)
plt.plot(x_values_pre, y_pre, linewidth=2, label='Pretrained B-spline Curve')
plt.title('Pretrained B-spline Curve and Basis Functions')
plt.legend(); plt.grid(); plt.show()

# ================================
#  Primera estimación spline vs datos (ya con cp pre-entrenados)
# ================================
...
class MyDeepSITAR(tf.keras.Model):
    ...
    def call(self, x):
        # x: entradas para estimar efectos aleatorios (usa Y como input)
...
# ===== Plot INICIAL (pre-entrenamiento aleatorio, solo para referencia visual) =====
bspline_layer_initial = BSplineLayer(num_seg=num_seg, degree=degree, domain=domain)
bspline_layer_initial.control_points.assign(init_cp)  # restaurar pesos ALEATORIOS

x_values_init = np.linspace(domain[0] - 4, domain[1] + 4, 200).astype(np.float32)
x_tensor_init = tf.constant(x_values_init, dtype=tf.float32)
y_init = bspline_layer_initial(x_tensor_init).numpy()

plt.figure(figsize=(10, 6))
basis_count_plot = num_seg + degree
for i in range(basis_count_plot):
    basis_y = bspline_layer_initial.bspline_basis_tensor(x_tensor_init, i, degree).numpy()
    plt.plot(x_tensor_init, basis_y, linestyle='--', label=f'Basis {i+1}')
plt.plot(x_tensor_init, y_init, linewidth=2, label='Initial (random) B-spline Curve')
plt.title('Initial (random) B-spline Curve and Basis Functions')
plt.xlabel('x'); plt.ylabel('Output'); plt.legend(); plt.grid(); plt.show()

# ================================
#  Entrenamiento principal (usa el spline ya pre-entrenado como INICIAL)
# ================================
DeepSITAR = MyDeepSITAR(x_ref=X, spline_layer=bspline_layer, deep_model=model_deep)
his_loss_train, his_loss_val = [], []

for i in range(len(lr)):
    opt = Adam(learning_rate=lr[i], epsilon=1e-7)  # epsilon más estable
...
# Curvas de pérdida (toda la cadena y últimos 1000)
...
plt.scatter(f_val.numpy().ravel(), yhat_val.numpy().ravel(),
...
plt.plot([min_val_xy, max_val_xy], [min_val_xy, max_val_xy],
         color='red', linestyle='--', label='x=f')
plt.xlabel("f")
plt.ylabel("Y_estim")
plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.1),
           fancybox=True, shadow=True, ncol=2)
plt.savefig(PLOTS_DIR / f'plot_compare_estim_{num_seg}.pdf', dpi=300, bbox_inches='tight')
plt.show()

# Visualización FINAL: bases + curva B-spline ENTRENADA (usa el mismo layer)
...
for i in range(basis_count):
    ...
    plt.plot(x_values, basis_y, linestyle='--', label=f'Basis {i+1}')
y_trained = bspline_layer(x_tensor).numpy()
plt.plot(x_values, y_trained, linewidth=2, label='Trained B-spline Curve')
plt.title('Trained B-spline Curve and Basis Functions')
plt.xlabel('x')
plt.ylabel('Output')
plt.legend()
plt.grid()
